...
function getMeaning(word: ArabicWord, lang: "hi" | "hinglish" | "en"): string {
...
export default function WordMeaningQuiz({ difficulty }: Props) {
  ...
  const loadQuestions = useCallback(() => {
    ...
    const pool = shuffle(WORD_LEVELS[difficulty]);
    const picked = pool.slice(0, TOTAL_QUESTIONS);
    const allMeanings = pool.map((w) => getMeaning(w, lang));
    ...
    for (const word of picked) {
      ...
      qs.push({
        ...
        correctMeaningEn: word.meaning_en,
  ...
  const isCorrect = selected === current.correctMeaning;
    ...
    <div>
      ...
      <div className="space-y-3 mb-6">
        {current.options.map((option, idx) => {
          ...
          } else if (option === current.correctMeaning) {
            className +=
          ...
          return (
            <button key={idx} className={className} onClick={() => handleSelect(option)}>
              <div className="flex items-start gap-3">
                ...
                <span>{option}</span>
                {selected && option === current.correctMeaning && (
                  <CheckCircle className="h-5 w-5 text-green-500 shrink-0 ml-auto" />
                )}
                {selected && option === selected && option !== current.correctMeaning && (
        ...
        >
          ...
          {!isCorrect && (
            <p className="text-sm text-muted-foreground mt-1">
              {t("correct_was", lang)}{" "}
              <span className="font-medium text-foreground">{current.correctMeaning}</span>
            </p>
          )}
          {lang !== "en" && (
            <p className="text-xs text-muted-foreground mt-1">
              English: <span className="font-medium">{current.correctMeaningEn}</span>
            </p>
          )}
        </div>
      )}
